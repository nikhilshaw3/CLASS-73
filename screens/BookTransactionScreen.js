...
this.setState({
  ...
  checkStudentEligibilityForBookIssue = async()=>{  
    ...
    var ifStudentEligible=""
    if(studentref.docs.length===0){
    this.setState({scannedStudentId:'', scannedBookId: ''})
    ifStudentEligible=false
    Alert.alert("THIS STUDENTID DOES'NOT EXIST IN THE DATBASE")
    }else{
      studentref.docs.map((doc)=>{
        var student=doc.data();
        if(student.numberOfBooksIssued<2){
          ifStudentEligible=true
        }else{
          ifStudentEligible=false,
...
ifStudentEligible=true
        ...
        ifStudentEligible= false
        Alert.alert("THE WASN'T ISSUED BY THE STUDENT")
